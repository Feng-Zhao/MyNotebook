...
因为Redis是基于内存的, 多线程切换上下文反而降低效率(因为除了持久化之外,工作进程中几乎没有和硬盘读写,所以单线程效率极高,单线程也可以IO多路复用), 其实redis不是单线程,但是使用的是单线程模型

Cpu不是性能瓶颈, 瓶颈在于内存,网络带宽等. 单线程完全足够使用
...
如果在 watch 执行后, key 被另一个线程修改过, 则事务会失败, 什么都不发生;
...
一种数据结构, 对查询参数以 hash 形式存储, 在控制层中进行校验, 如果不符合规则, 则丢弃, 避免底层数据库服务器的查询压力
